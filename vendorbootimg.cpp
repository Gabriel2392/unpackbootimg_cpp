...
std::optional<VendorBootImageInfo> UnpackVendorBootImage(std::ifstream& input,
    ...
    const uint32_t num_header_pages = utils::GetNumberOfPages(info.header_size, page_size);
    ...
    if (info.header_version > 3) {
        const uint32_t table_pages = utils::GetNumberOfPages(info.vendor_ramdisk_table_size, page_size);
        const uint64_t table_offset = page_size * (num_header_pages + 
        ...
        for (uint32_t i = 0; i < info.vendor_ramdisk_table_entry_num; ++i) {
            const uint64_t entry_offset = table_offset + (info.vendor_ramdisk_table_entry_size * i);
            ...
            image_entries.emplace_back(
                ramdisk_offset_base + entry.offset,
                entry.size,
                entry.output_name
            );
        ...
        // Handle bootconfig
        const uint64_t bootconfig_offset = page_size * (num_header_pages +
                                                       utils::GetNumberOfPages(info.vendor_ramdisk_size, page_size) +
                                                       utils::GetNumberOfPages(info.dtb_size, page_size) +
                                                       table_pages);
        image_entries.emplace_back(
    ...
    // Handle DTB
    if (info.dtb_size > 0) {
        const uint64_t dtb_offset = page_size * (num_header_pages +
                                                utils::GetNumberOfPages(info.vendor_ramdisk_size, page_size));
    ...
    if (info.header_version > 3 && !vendor_ramdisk_symlinks.empty()) {
        ...
        if (!utils::CreateDirectory(symlink_dir)) return std::nullopt;
...
std::vector<std::string> FormatMkbootimgArguments(const VendorBootImageInfo& info) {
